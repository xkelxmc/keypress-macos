...
// MARK: - HorizontalHistoryZone

/// Which of the mode's two zones an overlay window carries.
///
/// The zones never share a window. Each has a placement of its own and appears or leaves on
/// its own, so neither can move the other by so much as a pixel.
enum HorizontalHistoryZone: Equatable {
    case ribbon
    case commands
}
...
// MARK: - HorizontalHistoryCommandZoneView

/// The command zone: everything that produces no text — held modifiers, shortcuts,
/// arrows, Escape, Backspace — displayed as one combination, like Latest mode.
struct HorizontalHistoryCommandZoneView: View {
    var state: HorizontalHistoryState
    let config: KeypressConfig
    @Environment(\.blockPhase) private var blockPhase
    ...
    private var snapshot: CommandZoneSnapshot {
        CommandZoneSnapshot(
            keys: self.state.commandKeys,
            repeatCount: self.state.commandRepeatCount,
            repeatKeyID: self.state.commandRepeatKeyID,
            pressedIDs: self.state.pressedCommandKeyIDs)
    }

    var body: some View {
        BlockPresentationView(value: self.snapshot, isPresent: self.state.hasCommandKeys) { snapshot in
            KeyboardThemeContainer(config: self.config) {
                HStack(spacing: CGFloat(self.keyboardTheme.keySpacing)) {
...
#Preview("Command zone — ⌘V ×3") {
    @Previewable @State var rig = RibbonPreviewRig()
    HorizontalHistoryCommandZoneView(state: rig.state, config: .shared)
        .onAppear {
